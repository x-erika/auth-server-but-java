...
| **Hardening** | Client secrets Argon2id-hashed at rest (constant-time verify, dual-mode for legacy rows). Login timing-equalised against missing/disabled/unverified accounts (always runs Argon2). `/login` `return_to` and `/oauth/logout` `post_logout_redirect_uri` validated to block open redirects. Email + username normalised to lowercase on write and looked up via `LOWER(...)` so `Alice` / `alice` resolve to the same account. Signup uniqueness check + `em.flush()` translates `ConstraintViolationException` to 409 instead of 500. `pg_advisory_xact_lock` serialises bootstrap inserts across replicas. Hourly `@Scheduled` sweep of expired/revoked refresh tokens. |
...
- `%prod` profile requires `DB_*` and `REDIS_*` env vars (no fallbacks). `auth.cookie.secure=true` in prod so the session/CSRF cookies refuse plain HTTP. Behind a load balancer, set `AUTH_TRUST_FORWARDED=true` **and** `AUTH_TRUSTED_PROXIES=<LB CIDR>` so `X-Forwarded-For` is honoured only from the LB (defaults are off, so a direct client cannot spoof its source IP via XFF).
